...
class AddRowAction extends actions_1.Action {
    async performAction(instanceCtx) {
        ...
        for (const columnId of Object.keys(this.actionDef.columnValues)) {
            ...
            row[columnId] = await evalContextVarExpr_1.evalContextVarExpr({
                contextVar,
    ...
    validateColumnValue(designCtx, columnId) {
        ...
        const exprUtils = new mwater_expressions_1.ExprUtils(designCtx.schema, blocks_1.createExprVariables(designCtx.contextVars));
        // Check context var
        const contextVarExpr = this.actionDef.columnValues[columnId];
        let contextVar;
