    ...
    validate(designCtx) {
        ...
        for (const widgetCV of widget.contextVars) {
            ...
            if (contextVarValue.type == "ref") {
                ...
                const error = contextVarValues_1.validateContextVarValue(designCtx.schema, widgetCV, designCtx.contextVars, contextVarValue.value);
                if (error) {
                    return error;
                }
            }
        ...
        const err = embeddedExprs_1.validateEmbeddedExprs({
    ...
    async performAction(instanceCtx) {
        ...
        for (const pageCV of widget.contextVars) {
            ...
            if (contextVarValue.type == "ref") {
                ...
                const value = await evalContextVarExpr_1.evalContextVarExpr({
        ...
        // Get title
        let title = localization_1.localize(this.actionDef.title, instanceCtx.locale);
        if (title) {
            ...
            for (const ee of this.actionDef.titleEmbeddedExprs || []) {
                ...
                exprValues.push(await evalContextVarExpr_1.evalContextVarExpr({
                    contextVar,
                    contextVarValue: contextVar ? instanceCtx.contextVarValues[contextVar.id] : null,
                    ctx: instanceCtx,
                    expr: ee.expr
        ...
        const renderContextVarValue = (contextVar) => {
            ...
            function renderContextVarValueEditor() {
                ...
                if (cvv.type == "literal") {
                    ...
                    return react_1.default.createElement(contextVarValues_1.ContextVarValueEditor, { schema: props.schema, dataSource: props.dataSource, contextVar: contextVar, contextVarValue: cvv.value, availContextVars: [], onContextVarValueChange: value => { handleCVVChange({ ...cvv, value }); } });
                }
                if (cvv.type == "ref") {
                    const refOptions = props.contextVars
                        .filter(cv => areContextVarCompatible(cv, contextVar))
                        .map(cv => ({ value: cv.id, label: cv.name }));
                    return react_1.default.createElement(bootstrap_1.Select, { options: refOptions, value: cvv.contextVarId, onChange: value => { handleCVVChange({ ...cvv, contextVarId: value }); }, nullLabel: "Select Variable..." });
                }
