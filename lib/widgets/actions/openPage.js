...
var OpenPageAction = /** @class */ (function (_super) {
    ...
    OpenPageAction.prototype.performAction = function (options) {
        ...
        var _loop_2 = function (cvid) {
            ...
            var outerCV = options.contextVars.find(function (cv) { return cv.id == _this.actionDef.contextVarValues[cvid].contextVarId; });
            if (!outerCV) {
                throw new Error("Outer context variable not found");
            }
            // Get value 
            var outerCVValue = options.contextVarValues[outerCV.id];
            ...
            if (outerCV.type == "rowset") {
                outerCVValue = {
                    ...
                    op: "and",
                    table: outerCV.table,
                    exprs: _.compact([outerCVValue].concat(_.map(options.getFilters(outerCV.id), function (f) { return f.expr; })))
                };
            }
        ...
        // Get title
        var title = localization_1.localize(this.actionDef.title, options.locale);
        if (title) {
            // Get any embedded expression values
            var exprValues = _.map(this.actionDef.titleEmbeddedExprs || [], function (ee) { return options.getContextVarExprValue(ee.contextVarId, ee.expr); });
            // Format and replace
