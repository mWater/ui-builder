...
var OpenPageAction = /** @class */ (function (_super) {
    ...
    OpenPageAction.prototype.validate = function (designCtx) {
        ...
        var _loop_1 = function (widgetCV) {
            ...
            if (contextVarValue.type == "ref") {
                ...
                var error = contextVarValues_1.validateContextVarValue(designCtx.schema, widgetCV, designCtx.contextVars, contextVarValue.value);
                if (error) {
                    return { value: error };
                }
            }
                ...
                if (widgetCV.type == "rowset") {
                    return { value: "Not available for rowsets" };
                }
    ...
    OpenPageAction.prototype.performAction = function (instanceCtx) {
        return __awaiter(this, void 0, void 0, function () {
            var contextVarValues, widget, _loop_2, this_2, _i, _a, pageCV, _b, _c, globalContextVar, title, exprValues, _loop_3, _d, _e, ee, page;
            return __generator(this, function (_f) {
                switch (_f.label) {
                    case 0:
                        contextVarValues = {};
                        widget = instanceCtx.widgetLibrary.widgets[this.actionDef.widgetId];
                        _loop_2 = function (pageCV) {
                            var pageCVId, contextVarValue, outerCV, outerCVValue, widgetCV, contextVar, value;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        pageCVId = pageCV.id;
                                        contextVarValue = this_2.actionDef.contextVarValues[pageCVId];
                                        if (!(contextVarValue.type == "ref")) return [3 /*break*/, 1];
                                        outerCV = instanceCtx.contextVars.find(function (cv) { return cv.id == contextVarValue.contextVarId; });
                                        if (!outerCV) {
                                            throw new Error("Outer context variable not found");
                                        }
                                        outerCVValue = instanceCtx.contextVarValues[outerCV.id];
                                        ...
                                        return [3 /*break*/, 5];
                                    case 1:
                                        if (!(contextVarValue.type == "null")) return [3 /*break*/, 2];
                                        contextVarValues[pageCVId] = null;
                                        return [3 /*break*/, 5];
                                    case 2:
                                        if (!(contextVarValue.type == "literal")) return [3 /*break*/, 3];
                                        contextVarValues[pageCVId] = contextVarValue.value;
                                        return [3 /*break*/, 5];
                                    case 3:
                                        if (!(contextVarValue.type == "contextVarExpr")) return [3 /*break*/, 5];
                                        widgetCV = widget.contextVars.find(function (cv) { return cv.id == pageCVId; });
                                        contextVar = instanceCtx.contextVars.find(function (cv) { return cv.id == contextVarValue.contextVarId; });
                                    ...
                                    case 4:
                                        value = _a.sent();
                                        // Wrap in literal expression if not row
                                        contextVarValues[pageCVId] = widgetCV.type == "row" ? value : { type: "literal", valueType: widgetCV.type, idTable: widgetCV.idTable, value: value };
                                        _a.label = 5;
                        ...
                        this_2 = this;
                        _i = 0, _a = widget.contextVars;
                        _f.label = 1;
                    case 1:
                        if (!(_i < _a.length)) return [3 /*break*/, 4];
                        pageCV = _a[_i];
                        return [5 /*yield**/, _loop_2(pageCV)];
                    case 2:
                        _f.sent();
                        _f.label = 3;
                    case 3:
    ...
    OpenPageAction.prototype.renderEditor = function (props) {
        ...
        var widgetOptions = lodash_1.default.sortByAll(Object.values(props.widgetLibrary.widgets), "group", "name").map(function (w) { return ({ label: (w.group ? w.group + ": " : "") + w.name, value: w.id }); });
        ...
        var renderContextVarValue = function (contextVar) {
            var cvv = actionDef.contextVarValues[contextVar.id];
            var handleCVVTypeSelect = function (cvvType) {
            ...
            var handleCVVChange = function (newCVV) {
                props.onChange(immer_1.default(actionDef, function (draft) {
            ...
            function renderContextVarValueEditor() {
                ...
                if (cvv.type == "literal") {
                    ...
                    return react_1.default.createElement(contextVarValues_1.ContextVarValueEditor, { schema: props.schema, dataSource: props.dataSource, contextVar: contextVar, contextVarValue: cvv.value, availContextVars: [], onContextVarValueChange: function (value) { handleCVVChange(__assign(__assign({}, cvv), { value: value })); } });
                }
                if (cvv.type == "ref") {
                    var refOptions = props.contextVars
                        .filter(function (cv) { return areContextVarCompatible(cv, contextVar); })
                        .map(function (cv) { return ({ value: cv.id, label: cv.name }); });
                    return react_1.default.createElement(bootstrap_1.Select, { options: refOptions, value: cvv.contextVarId, onChange: function (value) { handleCVVChange(__assign(__assign({}, cvv), { contextVarId: value })); }, nullLabel: "Select Variable..." });
                }
                if (cvv.type == "contextVarExpr") {
                    return react_1.default.createElement(propertyEditors_1.ContextVarExprPropertyEditor, { contextVarId: cvv.contextVarId, contextVars: props.contextVars, dataSource: props.dataSource, expr: cvv.expr, schema: props.schema, types: [contextVar.type == "row" ? "id" : contextVar.type], idTable: contextVar.type == "row" ? contextVar.table : contextVar.idTable, onChange: function (contextVarId, expr) { handleCVVChange(__assign(__assign({}, cvv), { expr: expr, contextVarId: contextVarId })); } });
                }
        ...
        return (react_1.default.createElement("div", null,
            react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Page Type" },
                react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: this.actionDef, onChange: onChange, property: "pageType" }, function (value, onChange) { return react_1.default.createElement(bootstrap_1.Toggle, { value: value, onChange: onChange, options: [{ value: "normal", label: "Normal" }, { value: "modal", label: "Modal" }] }); })),
            this.actionDef.pageType == "modal" ?
                react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Modal Size" },
                    react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: this.actionDef, onChange: onChange, property: "modalSize" }, function (value, onChange) {
                        return react_1.default.createElement(bootstrap_1.Toggle, { value: value || "large", onChange: onChange, options: [
            ...
            react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: this.actionDef, onChange: onChange, property: "replacePage" }, function (value, onChange) { return react_1.default.createElement(bootstrap_1.Checkbox, { value: value, onChange: onChange }, "Replace current page"); }),
            ...
            react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Page Title" },
                react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: this.actionDef, onChange: onChange, property: "title" }, function (value, onChange) {
...
function areContextVarCompatible(cv1, cv2) {
    ...
    if (cv1.enumValues && cv2.enumValues) {
        if (!lodash_1.default.isEqual(cv1.enumValues.map(function (ev) { return ev.id; }), cv2.enumValues.map(function (ev) { return ev.id; }))) {
            return false;
        }
    }
    return true;
