    ...
    renderDesign(props) {
        const handleSet = (newBlockDef) => {
            if (newBlockDef) {
                ...
                const duplicatedBlockDef = blocks_1.duplicateBlockDef(newBlockDef, props.createBlock);
...
const AddWizardPane = (props) => {
    ...
    const [currentTabId, setCurrentTabId] = react_1.useState(defaultCurrentTabId);
    const [expressionMode, setExpressionMode] = react_1.useState(defaultExpressionMode);
    const [controlMode, setControlMode] = react_1.useState(defaultControlMode);
    // Focus on load
