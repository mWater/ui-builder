...
export class RowsetBlock extends CompoundBlock {
    ...
    renderInstance(props) {
        const contentNode = this.blockDef.content ?
            this.createBlock(this.blockDef.content).renderInstance(props) : React.createElement("div", null);
        const contextVar = this.createContextVar();
        // Inject context variable
        return React.createElement(ContextVarsInjector, { contextVars: [contextVar], contextVarValues: { [contextVar.id]: this.blockDef.filter }, createBlock: this.createBlock, database: props.database, innerBlock: this.blockDef.content, renderInstanceProps: props, schema: props.schema }, (renderInstanceProps, loading, refreshing) => {
            if (loading) {
    ...
    renderEditor(props) {
        ...
        const getOptionLabel = (table) => localize(table.name, props.locale);
        const getOptionValue = (table) => table.id;
        return (React.createElement("div", null,
            React.createElement(LabeledProperty, { label: "Table" },
                React.createElement(PropertyEditor, { obj: this.blockDef, onChange: props.onChange, property: "table" }, (value, onChange) => {
                    const handleTableChange = (table) => {
                        onChange(table.id);
                    };
                    return React.createElement(Select, { value: tables.find(t => t.id === value), onChange: handleTableChange, getOptionLabel: getOptionLabel, getOptionValue: getOptionValue });
                })),
            ...
            this.blockDef.table ?
                React.createElement(LabeledProperty, { label: "Filter" },
                    React.createElement(PropertyEditor, { obj: this.blockDef, onChange: props.onChange, property: "name" }, (value, onChange) => React.createElement(FilterExprComponent, { value: value, onChange: onChange, schema: props.schema, dataSource: props.dataSource, table: this.blockDef.table, variables: createExprVariables(props.contextVars) })))
                : null));
    }
}
