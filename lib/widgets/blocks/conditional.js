...
var ConditionalBlock = /** @class */ (function (_super) {
    ...
    ConditionalBlock.prototype.validate = function (ctx) {
        ...
        if (this.blockDef.contextVarId) {
            contextVar = ctx.contextVars.find(function (cv) { return cv.id === _this.blockDef.contextVarId && (cv.type === "rowset" || cv.type === "row"); });
            if (!contextVar) {
                return "Context variable required";
            }
        }
        var exprValidator = new mwater_expressions_1.ExprValidator(ctx.schema, blocks_1.createExprVariables(ctx.contextVars));
        // Validate expr
    ...
    ConditionalBlock.prototype.renderEditor = function (props) {
        ...
        return (React.createElement("div", null,
            ...
            React.createElement(propertyEditors_1.LabeledProperty, { label: "Conditional Expression" },
                React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "expr" }, function (value, onChange) {
                    return React.createElement(mwater_expressions_ui_1.ExprComponent, { value: value, onChange: onChange, schema: props.schema, dataSource: props.dataSource, aggrStatuses: ["individual", "aggregate", "literal"], types: ["boolean"], variables: blocks_1.createExprVariables(props.contextVars), table: contextVar ? contextVar.table || null : null });
                }))));
    };
