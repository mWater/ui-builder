...
class ToggleFilterBlock extends LeafBlock_1.default {
    ...
    canonicalize() {
        if (this.blockDef.forceSelection && this.blockDef.options.length > 0 && this.blockDef.initialOption == null) {
            return Object.assign(Object.assign({}, this.blockDef), { initialOption: 0 });
        }
        return this.blockDef;
    }
    renderDesign(props) {
        const options = this.blockDef.options.map((o, index) => ({ value: index, label: (0, localization_1.localize)(o.label, props.locale) }));
        return (react_1.default.createElement(bootstrap_1.Toggle, { options: options, value: this.blockDef.initialOption, onChange: (index) => props.store.replaceBlock(Object.assign(Object.assign({}, this.blockDef), { initialOption: index })), allowReset: !this.blockDef.forceSelection, size: mapToggleSize(this.blockDef.size) }));
    }
    renderInstance(ctx) {
        ...
        react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Filters", key: "filters" },
            react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: props.option, onChange: props.onChange, property: "filters" }, (value, onChange) => {
                ...
                return (react_1.default.createElement("div", null,
                    ...
                    react_1.default.createElement("button", { type: "button", className: "btn btn-link btn-sm", onClick: handleAddFilter }, "+ Add Filter")));
            }))));
}
...
function EditFilterComponent(props) {
    ...
    return (react_1.default.createElement("div", null,
        ...
        extraFilterCV ? (react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Filter expression" },
            react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: props.filter, onChange: props.onChange, property: "filterExpr" }, (value, onChange) => (react_1.default.createElement(mwater_expressions_ui_1.ExprComponent, { value: value, schema: props.schema, dataSource: props.dataSource, onChange: onChange, table: extraFilterCV.table, variables: (0, blocks_1.createExprVariables)(props.contextVars), types: ["boolean"] }))))) : null));
}
function ToggleFilterInstance(props) {
    const { blockDef, ctx } = props;
    const [selectedIndex, setSelectedIndex] = (0, react_1.useState)(blockDef.initialOption);
    // Set filter
    (0, react_1.useEffect)(() => {
    ...
    const options = blockDef.options.map((o, index) => ({ value: index, label: (0, localization_1.localize)(o.label, ctx.locale) }));
