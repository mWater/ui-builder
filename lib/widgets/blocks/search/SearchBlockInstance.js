...
const SearchBlockInstance = (props) => {
    ...
    const createExprFilter = (expr, searchText, table) => {
        ...
        if (exprType === "enum") {
            // Find matching enums
            const enumValues = exprUtils.getExprEnumValues(expr).filter(ev => (0, localization_1.localize)(ev.name, instanceCtx.locale).toLowerCase().includes(searchText.toLowerCase()));
