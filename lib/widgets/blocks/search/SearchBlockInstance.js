...
class SearchBlockInstance extends React.Component {
    constructor(props) {
        super(props);
    ...
    createExprFilter(expr, searchText, table) {
        ...
        const exprUtils = new mwater_expressions_1.ExprUtils(this.props.renderInstanceProps.schema, blocks_1.createExprVariables(this.props.renderInstanceProps.contextVars));
        ...
        if (exprType === "enum") {
            // Find matching enums
            const enumValues = exprUtils.getExprEnumValues(expr).filter(ev => localization_1.localize(ev.name, this.props.renderInstanceProps.locale).toLowerCase().includes(searchText.toLowerCase()));
