...
exports.registerExtraCodedPackage = void 0;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const req of requires) {
            if (req == "react") {
                ...
                importedModules[req] = yield Promise.resolve().then(() => __importStar(require("lodash")));
            }
            else if (req == "react-select") {
                importedModules[req] = yield Promise.resolve().then(() => __importStar(require("react-select")));
            }
            else if (req == "moment") {
                importedModules[req] = yield Promise.resolve().then(() => __importStar(require("moment")));
            }
            else if (req == "file-saver") {
                importedModules[req] = file_saver_1.default;
            }
            else if (req in extraPackages) {
                importedModules[req] = yield importedModules[req]();
            }
            else {
...
function CodedBlockInstance(props) {
    ...
    // Create props
    const compProps = { ctx: props.instanceCtx };
...
function CodedBlockDesign(props) {
    ...
    if (mod.DesignComp) {
        // Create props
        const compProps = { ctx: props.designCtx };
        // Add coded strings
        for (const codedLocalizedString of props.blockDef.codedLocalizedStrings || []) {
...
const CodedBlockEditModal = (props) => {
    ...
    (0, react_1.useEffect)(() => {
        Promise.resolve().then(() => __importStar(require("@babel/standalone"))).then((b) => {
            setBabel(b);
        });
        Promise.resolve().then(() => __importStar(require("@monaco-editor/react"))).then((m) => {
            setMonaco(m.default);
        });
    }, []);
    ...
    return (react_1.default.createElement(ModalWindowComponent_1.default, { isOpen: true, onRequestClose: handleSave }, Monaco ? (react_1.default.createElement(Monaco, { language: "javascript", height: "100%", onChange: handleChange, theme: "vs-dark", value: code, options: monacoOptions })) : null));
};
