...
class WidgetBlock extends LeafBlock_1.default {
    ...
    getContextVarExprs(contextVar, widgetLibrary, actionLibrary) {
        ...
        let contextVarExprs = _.flatten(blocks_1.getBlockTree(widgetDef.blockDef, this.createBlock, widgetDef.contextVars).map(cb => {
    ...
    renderInstance(props) {
        ...
        if (widgetDef && widgetDef.blockDef) {
            const innerBlock = this.createBlock(widgetDef.blockDef);
            const innerProps = Object.assign({}, props, { contextVars: props.contextVars.concat(widgetDef.contextVars), contextVarValues: Object.assign({}, props.contextVarValues, mappedContextVarValues), getContextVarExprValue: (contextVarId, expr) => {
                    // Lookup outer id
