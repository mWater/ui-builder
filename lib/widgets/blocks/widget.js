...
var WidgetBlock = /** @class */ (function (_super) {
    ...
    WidgetBlock.prototype.renderInstance = function (instanceCtx) {
        ...
        if (widgetDef && widgetDef.blockDef) {
            ...
            var innerContextVars = (instanceCtx.globalContextVars || [])
                ...
                .concat(widgetDef.contextVars);
            var innerContextVarValues = __assign(__assign(__assign({}, instanceCtx.contextVarValues), mappedContextVarValues), (_.pick(widgetDef.privateContextVarValues || {}, (widgetDef.privateContextVars || []).map(function (cv) { return cv.id; }))));
    ...
    WidgetBlock.prototype.renderEditor = function (props) {
        ...
        var widgetOptions = _.sortByAll(Object.values(props.widgetLibrary.widgets), "group", "name").map(function (w) { return ({ label: (w.group ? w.group + ": " : "") + w.name, value: w.id }); });
