...
class WidgetBlock extends LeafBlock_1.default {
    ...
    getContextVarExprs(contextVar, ctx) {
        ...
        let contextVarExprs = ctx.createBlock(widgetDef.blockDef).getSubtreeContextVarExprs(innerContextVar, Object.assign(Object.assign({}, ctx), { contextVars: widgetDef.contextVars }));
    ...
    mapInnerToOuterVariables(expr) {
        return (0, exports.mapObjectTree)(expr, (e) => {
            if (e.type === "variable") {
                // Change inner id to outer id
                if (this.blockDef.contextVarMap[e.variableId]) {
                    return Object.assign(Object.assign({}, e), { variableId: this.blockDef.contextVarMap[e.variableId] });
                }
                else {
                    return e;
                }
            ...
            if (e.type === "variable") {
                ...
                for (const key in this.blockDef.contextVarMap) {
                    if (this.blockDef.contextVarMap[key] == e.variableId) {
                        return Object.assign(Object.assign({}, e), { variableId: key });
                    }
                }
                return e;
            }
    ...
    renderDesign(props) {
        ...
        if (widgetDef && widgetDef.blockDef) {
            ...
            const innerProps = Object.assign(Object.assign({}, props), { selectedId: null, contextVars: innerContextVars, store: new blocks_1.NullBlockStore(), blockPaletteEntries: [], renderChildBlock: (childProps, childBlockDef) => {
                    if (childBlockDef) {
    ...
    renderInstance(instanceCtx) {
        ...
        if (widgetDef && widgetDef.blockDef) {
            ...
            const innerInstanceCtx = Object.assign(Object.assign({}, instanceCtx), { contextVars: innerContextVars, contextVarValues: innerContextVarValues, getContextVarExprValue: (contextVarId, expr) => {
                    ...
                    const outerContextVarId = this.blockDef.contextVarMap[contextVarId];
                    if (outerContextVarId) {
                        instanceCtx.setFilter(outerContextVarId, Object.assign(Object.assign({}, filter), { expr: this.mapInnerToOuterVariables(filter.expr) }));
                    }
                    ...
                    if (outerContextVarId) {
                        return instanceCtx
                            ...
                            .map((f) => (Object.assign(Object.assign({}, f), { expr: this.mapOuterToInnerVariables(f.expr) })));
                    }
                    return [];
                } });
    ...
    renderEditor(props) {
        ...
        const handleWidgetIdChange = (widgetId) => {
            props.store.replaceBlock(Object.assign(Object.assign({}, this.blockDef), { widgetId: widgetId, contextVarMap: {} }));
        };
        const renderContextVarValues = () => {
            ...
            return (React.createElement("table", { className: "table table-bordered table-sm" },
                React.createElement("tbody", null, widgetDef.contextVars.map((contextVar) => {
                    ...
                    const handleCVChange = (contextVarId) => {
                        props.store.replaceBlock((0, immer_1.default)(this.blockDef, (draft) => {
...
const mapObjectTree = (obj, mapping) => {
    // If array, map items
    if (_.isArray(obj)) {
        return obj.map((elem) => (0, exports.mapObjectTree)(elem, mapping));
    }
    if (_.isObject(obj)) {
