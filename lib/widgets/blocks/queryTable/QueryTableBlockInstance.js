...
export default class QueryTableBlockInstance extends React.Component {
    ...
    createRowRenderInstanceProps(rowIndex) {
        ...
        return Object.assign({}, rips, { contextVars: rips.contextVars.concat(rowcv), getContextVarValue: (cvid) => cvid === rowcv.id ? cvvalue : rips.getContextVarValue(cvid), getContextVarExprValue: (cvid, expr) => {
                if (cvid !== rowcv.id) {
