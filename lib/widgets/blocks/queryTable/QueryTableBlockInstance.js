...
var QueryTableBlockInstance = /** @class */ (function (_super) {
    ...
    QueryTableBlockInstance.prototype.createRowInstanceCtx = function (rowIndex) {
        ...
        var cvvalue = this.props.block.getRowContextVarValue(this.state.rows[rowIndex], rowExprs, this.props.instanceCtx.schema, rowsetCV, innerContextVars, contexts_1.getFilteredContextVarValues(this.props.instanceCtx)[rowsetCV.id]);
        var innerContextVarValues = __assign(__assign({}, rips.contextVarValues), (_a = {}, _a[rowcv.id] = cvvalue, _a));
        return __assign(__assign({}, rips), { contextVars: innerContextVars, contextVarValues: innerContextVarValues, getContextVarExprValue: function (cvid, expr) {
                ...
                if (cvid == null) {
                    return new mwater_expressions_1.PromiseExprEvaluator({
                        schema: rips.schema,
                        locale: rips.locale,
                        variables: blocks_1.createExprVariables(innerContextVars),
                        variableValues: blocks_1.createExprVariableValues(innerContextVars, innerContextVarValues)
                    }).evaluateSync(expr);
                }
                if (cvid !== rowcv.id) {
    ...
    QueryTableBlockInstance.prototype.renderRow = function (row, rowIndex) {
        ...
        var getColumnVerticalAlign = function (colIndex) {
            var columnInfos = _this.props.block.blockDef.columnInfos;
    ...
    QueryTableBlockInstance.prototype.render = function () {
        ...
        if (queryTable_1.getFixedWidth(this.props.block.blockDef)) {
            tableStyle.width = queryTable_1.getFixedWidth(this.props.block.blockDef);
            ...
            React.createElement("table", { className: className, style: tableStyle },
                ...
                blockDef.footers ?
                    React.createElement("tfoot", null,
                        React.createElement("tr", null, blockDef.footers.map(function (b, index) { return React.createElement("td", { key: index }, _this.props.instanceCtx.renderChildBlock(_this.props.instanceCtx, b)); })))
                    : null)));
    };
