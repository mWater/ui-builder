...
class QueryTableBlock extends blocks_1.Block {
    getChildren(contextVars) {
        ...
        const footerChildren = _.compact(this.blockDef.footers || []).map(bd => ({ blockDef: bd, contextVars: contextVars }));
    ...
    renderEditor(props) {
        ...
        const handleAddColumn = () => {
            props.store.replaceBlock(immer_1.default(this.blockDef, b => {
        ...
        return (React.createElement("div", null,
            ...
            React.createElement(propertyEditors_1.LabeledProperty, { label: "Mode" },
                React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "mode" }, (value, onChange) => React.createElement(bootstrap_1.Toggle, { value: value, onChange: onChange, options: [{ value: "singleRow", label: "One item per row" }, { value: "multiRow", label: "Multiple item per row" }] }))),
            rowsetCV ?
                React.createElement(propertyEditors_1.LabeledProperty, { label: "Filter" },
                    React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "where" }, (value, onChange) => (React.createElement(mwater_expressions_ui_1.ExprComponent, { value: value, onChange: onChange, schema: props.schema, dataSource: props.dataSource, types: ["boolean"], variables: blocks_1.createExprVariables(props.contextVars), table: rowsetCV.table }))))
                : null,
            rowCV ?
            ...
            React.createElement(propertyEditors_1.LabeledProperty, { label: "Maximum rows" },
                React.createElement("div", null,
                    ...
                    this.blockDef.limit != null ?
                        React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "limitType" }, (value, onChange) => React.createElement("div", { style: { paddingLeft: 10, display: "inline-block" } },
                            React.createElement(bootstrap_1.Checkbox, { value: value != "hard", onChange: v => onChange(v !== false ? "soft" : "hard") }, "Enable 'Show More...'")))
                        : null)),
            rowCV ?
...
const ColumnInfosEditor = (props) => {
    const handleOrderExprChange = (colIndex, expr) => {
        props.onChange(immer_1.default(props.value || [], draft => {
            // Make sure exists
    ...
    return React.createElement("ul", { className: "list-group" }, _.map(_.range(props.numColumns), colIndex => {
        return React.createElement("li", { className: "list-group-item", key: colIndex },
            ...
            React.createElement(propertyEditors_1.LabeledProperty, { label: "Vertical Alignment" },
                React.createElement(bootstrap_1.Toggle, { value: props.value && props.value[colIndex] ? props.value[colIndex].verticalAlign || "top" : "top", onChange: handleVerticalAlignChange.bind(null, colIndex), size: "xs", options: [
                        { value: "top", label: "Top" },
