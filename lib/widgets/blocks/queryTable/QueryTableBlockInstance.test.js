...
test("creates query", () => {
    ...
    const inst = (0, enzyme_1.mount)(React.createElement(QueryTableBlockInstance_1.default, { block: qtbSingle, instanceCtx: rips }));
...
test("adds filters, orderBy and where", () => {
    ...
    const qtb = createBlock(Object.assign(Object.assign({}, qtbdSingle), { where: { type: "literal", valueType: "boolean", value: false }, orderBy: [{ expr: { type: "field", table: "t1", column: "number" }, dir: "desc" }] }));
    const inst = (0, enzyme_1.mount)(React.createElement(QueryTableBlockInstance_1.default, { block: qtb, instanceCtx: rips }));
    const queryOptions = database.query.mock.calls[0][0];
    expect(queryOptions).toEqual({
        select: {
            id: { type: "id", table: "t1" },
