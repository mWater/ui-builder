...
const ActionLibrary_1 = require("../../ActionLibrary");
// Outer context vars
...
const qtbMultiple = new queryTable_1.QueryTableBlock(qtbdMultiple, createBlock);
...
test("gets row expressions", () => {
    ...
    const qtb = new queryTable_1.QueryTableBlock(qtbd, createBlock);
    expect(qtb.getRowExprs(contextVars, {}, {})).toEqual([expr]);
});
test("gets action expressions", () => {
    // Create simple action
    const expr = { type: "field", table: "t1", column: "text" };
    const qtbd = Object.assign({}, qtbdSingle, { rowClickAction: { type: "addRow", table: "t1", columnValues: { text: { contextVarId: "123_row", expr: expr } } } });
    const qtb = new queryTable_1.QueryTableBlock(qtbd, createBlock);
