...
var TOCBlock = /** @class */ (function (_super) {
    ...
    TOCBlock.prototype.getChildren = function (contextVars) {
        // Iterate all 
        return _.compact(iterateItems(this.blockDef.items).map(function (item) { return item.content; })).map(function (bd) { return ({ blockDef: bd, contextVars: contextVars }); });
    };
...
var TOCDesignComp = function (props) {
    ...
    return React.createElement(SplitPane, { left: blockDef.items.map(function (item, index) { return renderItem(blockDef.items, index, 0); }), right: renderProps.renderChildBlock(renderProps, selectedContent, handleSetContent) });
};
...
var TOCInstanceComp = function (props) {
    ...
    var selectedContent = selectedItem ? selectedItem.content : null;
    // Render overall structure
    return React.createElement(SplitPane, { left: blockDef.items.map(function (item, index) { return renderItem(blockDef.items, index, 0); }), right: renderProps.renderChildBlock(renderProps, selectedContent) });
};
