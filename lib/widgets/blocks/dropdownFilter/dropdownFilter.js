...
var DropdownFilterBlock = /** @class */ (function (_super) {
    ...
    DropdownFilterBlock.prototype.renderInstance = function (props) {
        ...
        var handleChange = function (newValue) {
            // Create filter
            var newFilter = _this.createFilter(_this.blockDef.rowsetContextVarId, _this.blockDef.filterExpr, props.schema, props.contextVars, newValue);
            props.setFilter(contextVar.id, newFilter);
            // Create extra filters
            for (var _i = 0, _a = _this.blockDef.extraFilters || []; _i < _a.length; _i++) {
                var extraFilter = _a[_i];
                var newExtraFilter = _this.createFilter(extraFilter.rowsetContextVarId, extraFilter.filterExpr, props.schema, props.contextVars, newValue);
        ...
        switch (valueType) {
            case "enum":
                ...
                break;
            case "enumset":
                elem = React.createElement(EnumsetInstance_1.default, { blockDef: this.blockDef, schema: props.schema, contextVars: props.contextVars, value: value, onChange: handleChange, locale: props.locale });
                break;
            case "text":
            ...
            case "datetime":
                var dateMode = this.blockDef.dateMode || "full";
                if (dateMode == "full") {
                    elem = React.createElement(DateExprComponent_1.default, { datetime: valueType == "datetime", table: contextVar.table, value: value, onChange: handleChange, placeholder: placeholder, locale: props.locale });
                }
                else {
                    elem = React.createElement(DateFilterInstance_1.DateFilterInstance, { mode: dateMode, value: value, onChange: handleChange, placeholder: placeholder, locale: props.locale });
                }
                break;
            case "id":
                elem = React.createElement(IdInstance_1.IdInstance, { blockDef: this.blockDef, ctx: props, value: value, onChange: handleChange, locale: props.locale });
                break;
            default:
