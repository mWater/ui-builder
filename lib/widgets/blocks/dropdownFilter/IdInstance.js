...
exports.IdInstance = (props) => {
    ...
    const formatIdLabel = react_1.useCallback((labelValues) => {
        if (props.blockDef.idMode == "advanced") {
            return embeddedExprs_1.formatEmbeddedExprString({
                text: localization_1.localize(blockDef.idLabelText, props.ctx.locale),
                contextVars: [],
    ...
    const onChange = hooks_1.useStabilizeFunction(props.onChange);
    const contextVars = hooks_1.useStabilizeValue(props.ctx.contextVars);
    const contextVarValues = hooks_1.useStabilizeValue(props.ctx.contextVarValues);
    return react_1.default.createElement(IdDropdownComponent_1.IdDropdownComponent, { database: props.ctx.database, table: idTable, value: props.value, onChange: onChange, multi: false, labelEmbeddedExprs: labelEmbeddedExprs, searchExprs: searchExprs, orderBy: orderBy, filterExpr: props.blockDef.idFilterExpr || null, formatLabel: formatIdLabel, placeholder: localization_1.localize(props.blockDef.placeholder, props.locale), contextVars: contextVars, contextVarValues: contextVarValues, styles: styles });
};
