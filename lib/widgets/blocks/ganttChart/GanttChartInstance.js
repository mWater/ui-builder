...
function GanttChartInstance(props) {
    ...
    const dbChanged = Database_1.useDatabaseChangeListener(ctx.database);
    ...
    const performQuery = () => __awaiter(this, void 0, void 0, function* () {
        ...
        const rows = yield ctx.database.query(createQuery(), ctx.contextVars, contexts_1.getFilteredContextVarValues(ctx));
        setRows(rows);
    });
    ...
    const handleAddRow = (parent, order) => {
        ...
        const innerCtx = immer_1.produce(ctx, draft => {
