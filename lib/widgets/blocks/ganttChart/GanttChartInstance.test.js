...
test("sorts simple list", () => {
    ...
    const chartRows = (0, GanttChartInstance_1.createChartRows)({ queryRows, getColor: () => "red", prefixNumber: false });
...
test("sorts nested list", () => {
    ...
    const chartRows = (0, GanttChartInstance_1.createChartRows)({ queryRows, getColor: () => "red", prefixNumber: true });
    expect(chartRows.map((r) => r.id)).toEqual(["1", "1a", "1b", "1ba", "2", "3"]);
    expect(chartRows.map((r) => r.level)).toEqual([0, 1, 1, 2, 0, 0]);
    expect(chartRows.map((r) => r.label)).toEqual(["1. A", "1.1. AA", "1.2. AB", "1.2.1. ABA", "2. B", "3. C"]);
});
