...
function TOCDesignComp(props) {
    ...
    const [selectedId, setSelectedId] = react_2.useState(blockDef.items[0] ? blockDef.items[0].id : null);
    // Select item
    const handleItemClick = (item) => { setSelectedId(item.id); };
    /** Alter items using an action */
    const alterBlockItems = (action) => {
        renderProps.store.replaceBlock(immer_1.default(blockDef, draft => {
    ...
    const handleAddItem = () => {
        renderProps.store.replaceBlock(immer_1.default(blockDef, draft => {
            draft.items.push({
                id: uuid_1.default(),
                labelBlock: { type: "text", id: uuid_1.default.v4(), text: { _base: renderProps.locale, [renderProps.locale]: "New Item" } },
                children: [],
                contextVarMap: {}
            });
        }));
    ...
    const addChildItem = (itemId) => {
        alterBlockItems((item) => {
            if (item.id === itemId) {
                return immer_1.default(item, draft => {
                    draft.children.push({
                        id: uuid_1.default(),
                        labelBlock: { type: "text", id: uuid_1.default.v4(), text: { _base: renderProps.locale, [renderProps.locale]: "New Item" } },
                        children: []
    ...
    return react_1.default.createElement(SplitPane_1.default, { left: renderLeft(), right: renderRight(), removePadding: false, theme: blockDef.theme || "light" });
}
