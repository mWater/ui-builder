...
exports.alterItems = (items, action) => {
    const newItems = _.flatten(_.compact(items.map(item => action(item))));
    return immer_1.produce(newItems, draft => {
...
class TOCBlock extends blocks_1.Block {
    ...
    getChildren(contextVars) {
        // Iterate all 
        return _.compact([this.blockDef.header, this.blockDef.footer].concat(exports.iterateItems(this.blockDef.items).map(item => item.labelBlock || null)))
    ...
    getContextVarExprs(contextVar, ctx) {
        return _.compact(exports.iterateItems(this.blockDef.items).map(item => item.condition && item.condition.contextVarId == contextVar.id ? item.condition.expr : null));
    }
    ...
    validate(designCtx) {
        // Validate all items
        for (const tocItem of exports.iterateItems(this.blockDef.items)) {
    ...
    processChildren(action) {
        return immer_1.produce(this.blockDef, (draft) => {
            // For header and footer
            draft.header = action(this.blockDef.header);
            draft.footer = action(this.blockDef.footer);
            // For all other blocks
            for (const item of exports.iterateItems(draft.items)) {
    ...
    renderEditor(props) {
        return (React.createElement("div", null,
            React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "removePadding" }, (value, onChange) => React.createElement(bootstrap_1.Checkbox, { value: value, onChange: onChange }, "Remove Padding (for top-level TOCs)")),
            ...
            React.createElement(propertyEditors_1.LabeledProperty, { label: "Collapse Below Width" },
                React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "collapseWidth" }, (value, onChange) => (React.createElement(bootstrap_1.Select, { value: value, onChange: v => onChange(v != null ? v : undefined), options: [
