...
class TOCBlock extends blocks_1.Block {
    ...
    getChildren(contextVars) {
        // Iterate all
        return _.compact([this.blockDef.header, this.blockDef.footer].concat((0, exports.iterateItems)(this.blockDef.items).map((item) => item.labelBlock || null))).map((bd) => ({ blockDef: bd, contextVars: contextVars }));
    }
