...
function TOCDesignRightPane(props) {
    ...
    const widgetOptions = lodash_1.default.sortByAll(Object.values(renderProps.widgetLibrary.widgets), "group", "name").map(w => ({ label: (w.group ? `${w.group}: ` : "") + w.name, value: w.id }));
            ...
            react_1.default.createElement("tbody", null, widgetDef.contextVars.map(contextVar => {
                ...
                const handleCVChange = (contextVarId) => {
                    if (contextVarId) {
                        ...
                        handleContextVarMapChange((0, immer_1.default)(contextVarMap, draft => { delete draft[contextVar.id]; }));
                    }
                };
    ...
    return (react_1.default.createElement("div", { style: { padding: 10 } },
        ...
        item.title ?
            react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Title embedded expressions", help: "Reference in text as {0}, {1}, etc." },
                react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: item, onChange: onItemChange, property: "titleEmbeddedExprs" }, (value, onChange) => (react_1.default.createElement(propertyEditors_1.EmbeddedExprsEditor, { value: value, onChange: onChange, schema: renderProps.schema, dataSource: renderProps.dataSource, contextVars: renderProps.contextVars }))))
            : null,
        ...
        item.children.length > 0 && item.collapse == "startExpanded" ?
            react_1.default.createElement(propertyEditors_1.LabeledProperty, { label: "Collapse at width" },
                react_1.default.createElement(propertyEditors_1.PropertyEditor, { obj: item, onChange: onItemChange, property: "collapseWidth" }, (value, onChange) => react_1.default.createElement(propertyEditors_1.ResponsiveWidthSelector, { value: value, onChange: onChange })))
            : null,
