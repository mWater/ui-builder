...
function TOCInstanceComp(props) {
    ...
    const [collapsedItems, setCollapsedItems] = (0, react_1.useState)(() => {
        return allItems
            .filter((item) => item.collapse == "startCollapsed" ||
    ...
    const renderItemTree = (items, index, depth) => {
        ...
        return (react_2.default.createElement("div", { key: item.id, className: `toc-item toc-item-level${depth}` },
            react_2.default.createElement("div", { key: "label", className: labelClasses.join(" "), onClick: handleItemClick.bind(null, item) },
                react_2.default.createElement("div", { key: "expand", className: "chevron", onClick: handleItemToggle.bind(null, item) }, collapsible ? (collapsed ? (react_2.default.createElement("i", { className: "fas fa-fw fa-caret-right" })) : (react_2.default.createElement("i", { className: "fas fa-fw fa-caret-down" }))) : (react_2.default.createElement("i", { className: "fas fa-fw fa-caret-right", style: { visibility: "hidden" } }))),
                renderItem(item)),
            item.children.length > 0 && !collapsed ? (react_2.default.createElement("div", { key: "children", className: "toc-item-children" }, item.children.map((child, index) => renderItemTree(item.children, index, depth + 1)))) : null));
    };
    function renderLeft() {
    ...
    function getTitle(item) {
        ...
        const exprValues = lodash_1.default.map(item.titleEmbeddedExprs || [], (ee) => instanceCtx.getContextVarExprValue(ee.contextVarId, ee.expr));
        // Format and replace
        let title = (0, localization_1.localize)(item.title, instanceCtx.locale);
        ...
        return (react_2.default.createElement("div", { onClick: () => setSelectorOpen(false) },
            react_2.default.createElement(FillDownwardComponent_1.default, null,
                react_2.default.createElement("div", { key: "header" },
                    ...
                    selectorOpen ? react_2.default.createElement("div", { className: "toc-selector" }, renderLeft()) : null),
                renderRight(true))));
    }
