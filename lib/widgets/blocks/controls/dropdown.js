...
class DropdownBlock extends ControlBlock_1.ControlBlock {
    ...
    renderControl(props) {
        ...
        if (!props.rowContextVar || !this.blockDef.column) {
            return react_1.default.createElement(react_select_1.default, { menuPortalTarget: document.body, classNamePrefix: "react-select-short", styles: { menuPortal: style => ({ ...style, zIndex: 2000 }) } });
        }
        // Get column
...
function IdDropdownInstance(props) {
    ...
    const formatIdLabel = hooks_1.useStabilizeFunction(props.formatIdLabel);
    const contextVars = hooks_1.useStabilizeValue(props.contextVars);
    const contextVarValues = hooks_1.useStabilizeValue(props.contextVarValues);
    const value = hooks_1.useStabilizeValue(props.value);
