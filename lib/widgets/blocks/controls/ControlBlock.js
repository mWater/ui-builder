...
var ControlBlock = /** @class */ (function (_super) {
    ...
    ControlBlock.prototype.renderControlEditor = function (props) {
        return null;
    };
    ControlBlock.prototype.renderDesign = function (designCtx) {
        var _this = this;
        var renderControlProps = {
            value: null,
            rowContextVar: designCtx.contextVars.find(function (cv) { return cv.id === _this.blockDef.rowContextVarId; }),
    ...
    ControlBlock.prototype.renderEditor = function (props) {
        ...
        return (React.createElement("div", null,
            ...
            React.createElement(collapsible_1.CollapsibleComponent, { label: "Optional Readonly Expression", initialCollapsed: true },
                React.createElement(propertyEditors_1.LabeledProperty, { label: "Readonly", hint: "optional expression that makes read-only if true" },
                    React.createElement(propertyEditors_1.PropertyEditor, { obj: this.blockDef, onChange: props.store.replaceBlock, property: "readonlyExpr" }, function (value, onChange) { return (React.createElement(propertyEditors_1.ContextVarExprPropertyEditor, { schema: props.schema, dataSource: props.dataSource, contextVars: props.contextVars, contextVarId: value ? value.contextVarId : null, expr: value ? value.expr : null, onChange: function (contextVarId, expr) { onChange({ contextVarId: contextVarId, expr: expr }); }, types: ["boolean"] })); })))));
    };
...
var ControlInstance = /** @class */ (function (_super) {
    ...
    function ControlInstance(props) {
        ...
        _this.validate = function (isFirstError) {
            // Check for null
            if (_this.getValue() == null && _this.props.block.blockDef.required) {
                _this.setState({ requiredError: _this.props.block.blockDef.requiredMessage ? localization_1.localize(_this.props.block.blockDef.requiredMessage, _this.props.instanceCtx.locale) : "" });
                // Scroll into view if first error
                if (isFirstError && _this.controlRef.current && _this.controlRef.current.scrollIntoView) {
                    _this.controlRef.current.scrollIntoView(true);
                    // Add some padding
                    var scrollParent = scrolling_1.getScrollParent(_this.controlRef.current);
                    if (scrollParent)
                ...
                _this.setState({ requiredError: null });
                return null;
            }
        };
    ...
    ControlInstance.prototype.render = function () {
        ...
        var readonly = blockDef.readonlyExpr ? instanceCtx.getContextVarExprValue(blockDef.readonlyExpr.contextVarId, blockDef.readonlyExpr.expr) : false;
        var renderControlProps = {
            value: this.getValue(),
        ...
        return (React.createElement("div", null,
            React.createElement("div", { style: style, ref: this.controlRef, key: "control" }, this.props.block.renderControl(renderControlProps)),
            this.state.requiredError ?
                React.createElement("div", { key: "error", className: "text-danger" }, this.state.requiredError)
                : null));
    };
