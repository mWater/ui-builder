...
export class DropdownFilterBlock extends LeafBlock {
    ...
    renderControl(options) {
        ...
        const getOptionLabel = (ev) => localize(ev.name, options.locale);
        const getOptionValue = (ev) => ev.id;
        const handleChange = (ev) => options.onChange(ev ? ev.id : null);
        return React.createElement(ReactSelect, { value: enumValue, onChange: handleChange, options: enumValues || undefined, placeholder: localize(this.blockDef.placeholder, options.locale), getOptionLabel: getOptionLabel, getOptionValue: getOptionValue, isClearable: true });
    }
    renderEditor(props) {
