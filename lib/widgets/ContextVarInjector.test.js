...
beforeEach(() => {
    const db = (0, mockDatabase_1.default)();
...
test("inner contains extra context vars", () => {
    ...
    const value = "1234";
    const contextVarExprs = [{ type: "field", table: "t1", column: "c1" }];
    let innerRenderProps;
    const x = (0, enzyme_1.shallow)(React.createElement(ContextVarInjector_1.default, { instanceCtx: outerRenderProps, injectedContextVar: contextVar, value: value, contextVarExprs: contextVarExprs }, (instanceCtx) => {
...
test("filters are applied for rowset variables", (done) => {
    const contextVar = { id: "cv1", name: "cv1", type: "rowset", table: "t1" };
    const value = { type: "literal", valueType: "boolean", value: false };
    const contextVarExprs = [{ type: "op", table: "t1", op: "count", exprs: [] }];
    const initialFilters = [{ id: "f1", expr: { type: "field", table: "t1", column: "c2" } }];
    let innerRenderProps;
    let innerIsLoading = false;
    // Need mount as shallow rendering fails to call lifecycle componentDidUpdate
    const x = (0, enzyme_1.mount)(React.createElement(ContextVarInjector_1.default, { instanceCtx: outerRenderProps, injectedContextVar: contextVar, value: value, contextVarExprs: contextVarExprs, initialFilters: Promise.resolve(initialFilters) }, (instanceCtx, isLoading) => {
        innerRenderProps = instanceCtx;
...
test("null filters are ignored for rowset variables", (done) => {
    ...
    const value = { type: "literal", valueType: "boolean", value: false };
    const contextVarExprs = [{ type: "op", table: "t1", op: "count", exprs: [] }];
    const initialFilters = [{ id: "f1", expr: null }];
    let innerRenderProps;
    let innerIsLoading = false;
    // Need mount as shallow rendering fails to call lifecycle componentDidUpdate
    const x = (0, enzyme_1.mount)(React.createElement(ContextVarInjector_1.default, { instanceCtx: outerRenderProps, injectedContextVar: contextVar, value: value, contextVarExprs: contextVarExprs, initialFilters: Promise.resolve(initialFilters) }, (instanceCtx, isLoading) => {
...
test("filters are not applied for rowset variables to variable value", (done) => {
    ...
    const value = { type: "literal", valueType: "boolean", value: false };
    const contextVarExprs = [{ type: "op", table: "t1", op: "count", exprs: [] }];
    const initialFilters = Promise.resolve([{ id: "f1", expr: { type: "field", table: "t1", column: "c2" } }]);
    let innerRenderProps;
    let innerIsLoading = false;
    // Need mount as shallow rendering fails to call lifecycle componentDidUpdate
    const x = (0, enzyme_1.mount)(React.createElement(ContextVarInjector_1.default, { instanceCtx: outerRenderProps, injectedContextVar: contextVar, value: value, contextVarExprs: contextVarExprs, initialFilters: initialFilters }, (instanceCtx, isLoading) => {
        innerRenderProps = instanceCtx;
        innerIsLoading = isLoading;
        return React.createElement("div", null);
    }));
