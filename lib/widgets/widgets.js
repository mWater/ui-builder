...
function validateWidget(widgetDef, ctx, includeChildren) {
    ...
    if (includeChildren) {
        ...
        for (const childBlock of (0, blocks_1.getBlockTree)(widgetDef.blockDef, ctx.createBlock, contextVars, ctx.schema)) {
            const block = ctx.createBlock(childBlock.blockDef);
            // Create design context for validating block
            const blockDesignCtx = Object.assign(Object.assign({}, ctx), { dataSource: ctx.dataSource, contextVars: childBlock.contextVars, store: new blocks_1.NullBlockStore(), blockPaletteEntries: [], selectedId: null, renderChildBlock: () => {
