...
function validateWidget(widgetDef, ctx, includeChildren) {
    ...
    for (const cv of widgetDef.contextVars) {
        const error = contextVarValues_1.validateContextVarValue(ctx.schema, cv, globalContextVars.concat(widgetDef.contextVars), widgetDef.contextVarPreviewValues[cv.id]);
        if (error) {
            return error;
        }
    }
    ...
    for (const cv of privateContextVars) {
        const error = contextVarValues_1.validateContextVarValue(ctx.schema, cv, globalContextVars.concat(widgetDef.contextVars.concat(privateContextVars)), (widgetDef.privateContextVarValues || {})[cv.id]);
        if (error) {
            return error;
        }
    }
    if (includeChildren) {
        ...
        for (const childBlock of blocks_1.getBlockTree(widgetDef.blockDef, ctx.createBlock, contextVars)) {
            const block = ctx.createBlock(childBlock.blockDef);
            // Create design context for validating block
            const blockDesignCtx = {
                ...ctx,
