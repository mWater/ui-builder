 ...
 */
async function evalContextVarExpr(options) {
    ...
    if (contextVar.type == "row") {
        ...
        const filteredContextVarValues = contexts_1.getFilteredContextVarValues(ctx);
    ...
    if (contextVar.type == "rowset") {
        ...
        const aggrStatus = new mwater_expressions_1.ExprUtils(ctx.schema, blocks_1.createExprVariables(ctx.contextVars)).getExprAggrStatus(expr);
        // Create query to get value
        const queryOptions = {
            select: {
                value: expr
