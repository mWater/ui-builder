...
class Block {
    ...
    getInitialFilters(contextVarId, instanceCtx) { return []; }
...
function duplicateBlockDef(blockDef, createBlock) {
    return createBlock(blockDef).process(createBlock, (bd) => bd ? (Object.assign(Object.assign({}, bd), { id: (0, uuid_1.v4)() })) : null);
}
...
function validateContextVarExpr(options) {
    ...
    const aggrStatuses = options.aggrStatuses || ((contextVar && contextVar.type == "row") ? ["individual", "literal"] : ['aggregate', 'individual', 'literal']);
    // Validate expr
