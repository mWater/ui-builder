...
export default class VirtualDatabase {
    ...
    async query(options, contextVars, contextVarValues) {
        const variables = createExprVariables(contextVars);
        const variableValues = contextVarValues;
        // Create rows to evaluate
        let evalRows = (await this.queryEvalRows(options.from, options.where || null, contextVars, contextVarValues)).map(r => ({ row: r }));
    ...
    async queryEvalRows(from, where, contextVars, contextVarValues) {
    ...
    async mutateRows(rows, from, where, contextVars, contextVarValues) {
