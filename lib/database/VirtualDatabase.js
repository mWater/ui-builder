...
class VirtualDatabase {
    ...
    query(query, contextVars, contextVarValues) {
        return __awaiter(this, void 0, void 0, function* () {
            const variables = (0, blocks_1.createExprVariables)(contextVars);
            const variableValues = (0, blocks_1.createExprVariableValues)(contextVars, contextVarValues);
            ...
            return (0, Database_1.performEvalQuery)({ evalRows, exprUtils, exprEval, query: query });
        });
    }
    ...
    doesReferenceTempPk(expr, exprUtils, contextVars, contextVarValues) {
        ...
        const inlinedExpr = exprUtils.inlineVariableValues(expr, (0, blocks_1.createExprVariableValues)(contextVars, contextVarValues));
    ...
    queryEvalRows(from, where, contextVars, contextVarValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const updatedIds = lodash_1.default.uniq(this.mutations.filter((m) => m.type == "update" && m.table == from).map((m) => m.primaryKey));
            ...
            // Perform query
            let rows;
            const wherePrimaryKey = (0, Database_1.getWherePrimaryKey)(where);
            const wherePrimaryKeys = (0, Database_1.getWherePrimaryKeys)(where);
            ...
            // Adjust filter if query on mix of primary keys and temporary primary keys
                ...
                if (nonTempKeys.length > 0) {
                    const newQueryOptions = (0, immer_1.default)(queryOptions, (draft) => {
                        draft.where = {
                            type: "op",
                            op: "= any",
                            table: from,
                    ...
                    rows = yield this.queryCache.get({
...
class VirtualDatabaseTransaction {
    ...
    addRow(table, values) {
        ...
        const primaryKey = `pk_${tempPrimaryKeyCounter.toString().padStart(6, "0")}${(0, uuid_1.v4)().replace(/-/g, "")}_temp`;
    ...
    commit() {
        ...
        for (const mutation of this.mutations) {
            if (mutation.type == "add") {
                ...
                const existingAdd = this.virtualDatabase.mutations.find((m) => m.table == mutation.table && m.type == "add" && m.primaryKey == mutation.primaryKey);
                if (existingAdd) {
                    existingAdd.values = Object.assign(Object.assign({}, existingAdd.values), mutation.updates);
                    continue;
                }
                // Combine with update if present
                const existingUpdate = this.virtualDatabase.mutations.find((m) => m.table == mutation.table && m.type == "update" && m.primaryKey == mutation.primaryKey);
                if (existingUpdate) {
                ...
                // Combine with add if present
                const existingAddIndex = this.virtualDatabase.mutations.findIndex((m) => m.table == mutation.table && m.type == "add" && m.primaryKey == mutation.primaryKey);
