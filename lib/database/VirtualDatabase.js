...
class VirtualDatabase {
    ...
    query(options, contextVars, contextVarValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let evalRows = (yield this.queryEvalRows(options.from, options.where || null, contextVars, contextVarValues)).map(r => ({ row: r }));
            ...
            const exprEval = new PromiseExprEvaluator_1.PromiseExprEvaluator(new mwater_expressions_1.ExprEvaluator(this.schema, this.locale, variables, variableValues));
            // Evaluate all non-aggr selects and non-aggr orderbys
            for (const evalRow of evalRows) {
                for (let i = 0; i < selects.length; i++) {
                    if (!selects[i].isAggr) {
                        evalRow["s" + i] = yield exprEval.evaluate(selects[i].expr, { row: evalRow.row });
                    }
                }
                for (let i = 0; i < orderBys.length; i++) {
                    if (!orderBys[i].isAggr) {
                        evalRow["o" + i] = yield exprEval.evaluate(orderBys[i].expr, { row: evalRow.row });
                    }
                }
            }
            ...
            if (selects.find(s => s.isAggr) || orderBys.find(o => o.isAggr)) {
                ...
                for (const group of Object.values(groups)) {
                    ...
                    for (let i = 0; i < selects.length; i++) {
                        if (selects[i].isAggr) {
                            evalRow["s" + i] = yield exprEval.evaluate(selects[i].expr, { row: evalRow.row, rows: group.map(r => r.row) });
                        }
                    }
                    for (let i = 0; i < orderBys.length; i++) {
                        if (orderBys[i].isAggr) {
                            evalRow["o" + i] = yield exprEval.evaluate(orderBys[i].expr, { row: evalRow.row, rows: group.map(r => r.row) });
                        }
                    }
                }
            ...
            for (const mutation of this.mutations) {
                switch (mutation.type) {
                    case "add":
                        ...
                        const primaryKey = yield txn.addRow(mutation.table, mutation.values);
                    ...
                    case "update":
                        ...
                        yield txn.updateRow(mutation.table, mutation.primaryKey, mutation.updates);
                        break;
                    case "remove":
                        yield txn.removeRow(mutation.table, mutation.primaryKey);
                        break;
                }
            }
            yield txn.commit();
    ...
    queryEvalRows(from, where, contextVars, contextVarValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // Perform query
            let rows;
            const queryCacheKey = canonical_json_1.default({ queryOptions: queryOptions, contextVars: contextVars, contextVarValues: contextVarValues });
            rows = this.cache.get(queryCacheKey);
            if (!rows) {
                rows = yield this.database.query(queryOptions, contextVars, contextVarValues);
    ...
    mutateRows(rows, from, where, contextVars, contextVarValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (where) {
                const filteredRows = [];
                const exprEval = new PromiseExprEvaluator_1.PromiseExprEvaluator(new mwater_expressions_1.ExprEvaluator(this.schema));
