 ...
 */
async function performEvalQuery(options) {
    ...
    if (query.where) {
        ...
        const whereValues = await Promise.all(wherePromises);
...
function stableSort(items, iteratee, direction) {
    return stable_1.default(items, (a, b) => direction == "asc" ? normalCompare(iteratee(a), iteratee(b)) : normalCompare(iteratee(b), iteratee(a)));
}
...
function useDatabaseChangeListener(database) {
    const [incr, setIncr] = react_1.useState(0);
