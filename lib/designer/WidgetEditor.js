...
var WidgetEditor = /** @class */ (function (_super) {
    ...
    WidgetEditor.prototype.validateWidget = function () {
        var widgetDef = this.props.widgetDef;
        // Validate context var values
        for (var _i = 0, _a = widgetDef.contextVars; _i < _a.length; _i++) {
            var cv = _a[_i];
            var error = contextVarValues_1.validateContextVarValue(this.props.designCtx.schema, cv, widgetDef.contextVars, widgetDef.contextVarPreviewValues[cv.id]);
            if (error) {
                return error;
            }
        }
        ...
        for (var _b = 0, _c = widgetDef.privateContextVars || []; _b < _c.length; _b++) {
            var cv = _c[_b];
            var error = contextVarValues_1.validateContextVarValue(this.props.designCtx.schema, cv, widgetDef.privateContextVars.concat(widgetDef.contextVars), (widgetDef.privateContextVarValues || {})[cv.id]);
            if (error) {
                return error;
            }
        }
    ...
    WidgetEditor.prototype.render = function () {
        ...
        var validationError = this.validateWidget();
