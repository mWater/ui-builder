    ...
    function WidgetLibraryDesigner(props) {
        ...
        _this.validateWidget = function (widgetDef) {
            ...
            for (var _b = 0, _c = widgetDef.privateContextVars || []; _b < _c.length; _b++) {
                var cv = _c[_b];
                var error = contextVarValues_1.validateContextVarValue(_this.props.baseCtx.schema, cv, widgetDef.privateContextVars, (widgetDef.privateContextVarValues || {})[cv.id]);
                if (error) {
                    return error;
                }
            }
