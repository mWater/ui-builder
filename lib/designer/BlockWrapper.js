...
const blockTargetSpec = {
    ...
    drop(props, monitor, component) {
        ...
        setTimeout(() => {
            props.store.alterBlock(targetBlockDef.id, (b) => blocks_1.dropBlock(sourceBlockDef, b, dropSide), sourceBlockDef.id);
        }, 0);
    }
};
...
const dropTarget = react_dnd_1.DropTarget("block", blockTargetSpec, (connect, monitor) => ({
    connectDropTarget: connect.dropTarget(),
    isOver: monitor.isOver({ shallow: true }),
    canDrop: monitor.canDrop()
}))(BlockWrapper);
exports.default = react_dnd_1.DragSource("block", blockSourceSpec, (connect, monitor) => ({
