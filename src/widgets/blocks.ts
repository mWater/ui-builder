...
export abstract class Block<T extends BlockDef> {
  ...
  async getSubtreeInitialFilters(contextVarId: string, instanceCtx: InstanceCtx): Promise<Filter[]> {
    // Get own filters
    let ownFilters = await this.getInitialFilters(contextVarId, instanceCtx)
    ...
    for (const childBlock of this.getChildren(instanceCtx.contextVars, instanceCtx.schema)) {
      ...
      ownFilters = ownFilters.concat(await block.getSubtreeInitialFilters(contextVarId, instanceCtx))
    }
    return ownFilters
  }
