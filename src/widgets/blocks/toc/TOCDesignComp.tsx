...
export default function TOCDesignComp(props: { blockDef: TOCBlockDef; renderProps: DesignCtx }) {
  ...
  const alterBlockItems = (action: (item: TOCItem) => TOCItem | TOCItem[] | undefined | null) => {
    renderProps.store.replaceBlock(
      produce(blockDef, (draft) => {
  ...
  function renderItem(item: TOCItem, index: number, depth: number, dragHandleProps?: DraggableProvidedDragHandleProps) {
    ...
    return (
      <div className={`toc-item toc-item-level${depth}`}>
        <div
          key="main"
        ...
        {item.children.length > 0 ? (
          <div key="children">{renderItems(item.children, depth + 1, handleSetChildren.bind(null, item.id))}</div>
        ) : null}
      </div>
    )
  }
  ...
  return <SplitPane left={renderLeft()} right={renderRight()} theme={blockDef.theme || "light"} />
}
