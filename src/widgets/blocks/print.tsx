...
const PrintInstance = (props: { blockDef: PrintBlockDef; ctx: InstanceCtx }) => {
  ...
  return (
    <div>
      ...
      {printing ? (
        <ExecutePrintInstance blockDef={props.blockDef} ctx={props.ctx} onComplete={() => setPrinting(false)} />
      ) : null}
    </div>
  )
}
...
const ExecutePrintInstance = (props: {
  ...
  const printElem = useMemo(() => {
    ...
    return (
      <div style={{ transform: "scale(0.5)", transformOrigin: "top left" }}>
        <div style={{ width: width }}>{printCtx.renderChildBlock(printCtx, props.blockDef.content)}</div>
      </div>
    )
  }, [printCtx])
  ...
  const sizeCss =
    (props.blockDef.paperSize || "letter-portait") == "letter-portait" ? "8.5in 11in" : "11in 8.5in landscape"
