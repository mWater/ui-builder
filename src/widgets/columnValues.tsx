...
export class ColumnValuesEditor extends React.Component<{
        ...
        <td key="value">
          <LabeledProperty label="Expression">
            <ContextVarAndExprPropertyEditor
              ...
              aggrStatuses={
                contextVar && contextVar.type == "rowset" ? ["aggregate", "literal"] : ["individual", "literal"]
              }
              types={[type]}
            />
          </LabeledProperty>
  ...
  render() {
    ...
    const columns = this.props.schema
      ...
      .filter(
        (column) => !column.expr && (column.type != "join" || column.join!.type == "1-1" || column.join!.type == "n-1")
      )
    const options = _.sortBy(
      columns.map((column) => ({ value: column.id, label: localize(column.name, this.props.locale) })),
      "label"
    )
