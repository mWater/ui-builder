...
export default class ListEditor<T> extends React.Component<ListEditorProps<T>> {
  ...
  renderItem(item: T, index: number) {
    return (
      <li className="list-group-item" key={index}>
        <span style={{ float: "right" }} className="text-primary" onClick={this.handleRemove.bind(null, index)}>
  ...
  render() {
    return <ul className="list-group">{this.props.items.map((item, index) => this.renderItem(item, index))}</ul>
  }
}
